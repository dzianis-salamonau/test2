	...
	<header class="entry-header">
		...
		<?php
			if ( is_front_page() ) :
				...
				<h1 class="entry-title"><?php wp_title();//wp_title( '<h1 class="entry-title">', '</h1>' ); 
				?></h1>
				</div>
				<?php
			endif;
...
$(document).ready(function(){
	...
	$('#search-toggle').click(function() {
		...
		$('#mega-menu-menu-1 li').each(function () {
		var link = $(this).find('a').attr('href');

		if (location == link)
		{
		    $(this).children('a').addClass('current-link');
		}
		});
    ...
    function checkScreenSize(){
        ...
        if (newWindowWidth > 1030) {
		   var top = wnd.scrollTop();
	
		   if (top > 250)
			{
				opacityControl.addClass("opasity_zero");
				$('#nav_menu_wraper').css('top','0');
			}
		   else 
			{
				opacityControl.removeClass("opasity_zero");
				$('#nav_menu_wraper').css('top','36px');
		    	}
		wnd.scroll(function(){
		   var top = wnd.scrollTop();
